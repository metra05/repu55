...
#define DHTPIN 2   //D3   //0    //5     // Digital pin connected to the DHT sensor
...
const char* ap_ssid = "ESP8266_AP_LOG";
...
//const int ledPin = 2;
const int kedip = 16;
const int BUZZ = 0;     //ingat gpio0 harus kondisi high saat boot!!!!! hati hati memihlih pin    //coba dipakai dht
const int rel = 200;   //ini dipakai oleh relay pnp //atau rencananya dipakai dht
const int RESET_BUTTON_PIN = 15; // 12; 15; //awas pin lima belas butuh low saat booting //===> dipakai relay npn juga
...
const char* logFileNamelama = "/data_log_old.txt";
const unsigned long logInterval = 360000; // Interval pencatatan data (ms)
unsigned long lastLogTime = 0;
float sensorValue = 0;
const size_t maxLogSize = 2048;  //3072;     //2048;      //5120; //1024; //ini seberapa besar? 
...
*/
  ...
  if (!SPIFFS.exists(logFileName)) {
    ...
    if (!logFile) {
      ...
      logFile.println("Waktu, Data suhu, Data humidity -");
      logFile.close();
      Serial.println("File log dibuat");
    }
  }
...
//===============================ini punyanya log data=========================
// Baca sensor atau data yang ingin dicatat (contoh)
//  float sensorValue = 0;//analogRead(A0) / 1024.0 * 3.3; // Contoh membaca sensor
  
  // Catat data secara berkala
  if (millis() - lastLogTime > 60000){     //logInterval) {
    
     //===================================================================
    // ini punyanya lm35
    //float temperature = analogRead(A0);
    //temperature = (temperature*resolution);
    //temperature = (temperature*100)+27; //kebaca 2.26????? selisih berapa liniern kalibrasinya? harus ditambah 27 erorr

    //ini pakai ds------------------------
/*    rtc.convertTemperature();             //convert current temperature into registers    //awas baca suhu rtc ketika dsnya error 45/165
    Serial.print(rtc.getTemperature()); //read registers and display the temperature
    temperature=rtc.getTemperature();
//    Serial.println(temperature);          */
    //===================================================================
    
//    sensorValue = random(100);
    //logData(sensorValue);
    //Serial.println(temperature);
//    char bufft[30];
//    sprintf(bufft, "suhu: %0.1f", t);
//    char buffh[30];
//    sprintf(buffh, "humy: %0.1f", h);
    logDataWithRotation(t , h);   //millis(), t, h);    //temperature);  //sensorValue); //ini menggunakan pembatasan pada ukuran file.
    lastLogTime = millis();
